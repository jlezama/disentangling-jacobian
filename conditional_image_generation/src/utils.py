...
def get_optimizer(model, s):
    ...
    #return optim_fn([{'params':model.enc_layers.parameters()}, {'params':model.dec_layers.parameters()}], **optim_params)
...
def reload_model(model, to_reload, attributes=None):
    ...
    for k in model.state_dict().keys():
        if model.state_dict()[k].size() != to_reload.state_dict()[k].size():
            # copy manually
            their_model = to_reload.state_dict()[k]
            my_model = model.state_dict()[k]
            sz = their_model.size()
            print 'reload_model, ', my_model.size(), their_model.size()
            if len(sz)==4:
                my_model[0:sz[0],0:sz[1],:,:] = their_model[:,:,:,:]
            else:
                my_model[0:sz[0]] = their_model[:]
...
def reload_model2(model, to_reload, attributes=None):
    """
    Reload a previously trained model and appends at the end.
    """
    # reload the model
    assert os.path.isfile(to_reload)
    ...
    #print model_params, to_reload_params
    ...
    for k in model.state_dict().keys():
        if model.state_dict()[k].size() != to_reload.state_dict()[k].size():
            ...
            my_model = model.state_dict()[k]
            sz = their_model.size()
            print my_model.size(), their_model.size()
            if len(sz)==4:
                my_model[-sz[0]:,-sz[1]:,:,:] = their_model[:,:,:,:]
            else:
